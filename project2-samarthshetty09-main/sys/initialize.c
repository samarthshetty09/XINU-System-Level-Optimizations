...
#include <io.h>
#include <stdio.h>
#include <sched.h>  // Include the scheduler header
...
int currClassSchd = DEFAULTSCHD;  // Initialize the scheduling class to default
/* int epoch = 0;                     // Initialize epoch to 0
int preempt_counter = 0;           // Initialize preempt_counter to 0
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	/* Initialize NULL process's scheduler fields */
    pptr->quantum = 0;
    pptr->counter = 0;
    pptr->goodness = 0;
	...
	/* Initialize global scheduler variables */
    currClassSchd = DEFAULTSCHD;
/*     epoch = 0;
    preempt_counter = 0; */
