...
sysinit()
{
	...
	/* Initialize the SC page replacement policy queue */
    sc_head = 0;
    sc_tail = -1;
    sc_count = 0;
    for (i = 0; i < NFRAMES; i++) {
        sc_queue[i] = -1; // Initialize queue entries to -1 (empty)
    }

	// /* Initializing the backing store map */
    init_bsm();

    // /* Initializing the frame table */
    init_frm();

    // /* Initializing the paging system */
    init_paging();
