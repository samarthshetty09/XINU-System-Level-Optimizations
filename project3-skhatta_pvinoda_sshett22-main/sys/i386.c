...
setsegs()
{
    ...
    /* Set maxaddr to 8 MB - 1 */
    maxaddr = (char *)((2048 * NBPG) - 1);

    /* Set the code segment limit to maximum */
    psd = &gdt_copy[1];                   /* kernel code segment */
    np = ((int)&etext + NBPG - 1) / NBPG; /* # code pages */
    psd->sd_lolimit = np;
    psd->sd_hilimit = np >> 16;
    /* Other fields remain as initialized */

    /* Set the data segment limit to maximum */
    psd = &gdt_copy[2]; /* kernel data segment */
    psd->sd_lolimit = 0xFFFF;
    psd->sd_hilimit = 0xF;
    psd->sd_gran = 1; /* 4KB granularity */
    psd->sd_32b = 1;  /* 32-bit segment */
    psd->sd_present = 1;
    /* Other fields remain as initialized */

    /* Set the stack segment limit to maximum */
    ...
    /* Rest of setsegs() remains the same */

    /* Initialize initsp to the highest physical address */
    initsp = (2048 * NBPG) - 4; /* 8 MB - 4 bytes */

    /* Copy the modified GDT to the actual GDT */
